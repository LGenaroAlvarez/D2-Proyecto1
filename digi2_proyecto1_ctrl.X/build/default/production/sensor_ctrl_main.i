...
# 2 "<built-in>" 2
# 1 "sensor_ctrl_main.c" 2
# 12 "sensor_ctrl_main.c"
...
# 31 "sensor_ctrl_main.c" 2


# 1 "C:\\Program Files\\Microchip\\xc8\\v2.35\\pic\\include\\c90\\stdint.h" 1 3
# 33 "sensor_ctrl_main.c" 2
...
void initAdc (uint8_t adcs, uint8_t vref_pos, uint8_t vref_neg);
...
# 40 "sensor_ctrl_main.c" 2
# 53 "sensor_ctrl_main.c"
uint8_t Fosc = 1;
int PS_val = 16;
uint8_t cont = 0;
uint8_t milis = 0;
...
uint16_t cont1=0x00, data = 0, response = 0;
...
unsigned short RTC_sec = 0;
unsigned short RTC_min = 0;
unsigned short RTC_hor = 0;
char RTC_sec_str[10];
char RTC_min_str[10];
char RTC_hor_str[10];
...
int percent(uint16_t val, uint16_t water_val, uint16_t air_val);
...
void __attribute__((picinterrupt(("")))) isr(void){
    ...
    if (ADIF){
        if (ADCON0bits.CHS == 0){
            HS_val = adcRead();
            HS_percent = percent(HS_val, 219, 474);
            sprintf(HS_str, "%d", HS_percent);
        }
        PIR1bits.ADIF = 0;
    }
...
void main(void) {
    ...
    while(1){
        ...
        I2C_Master_Start();
        I2C_Master_Write(0b1001000 +0b1);
        ...
        sprintf(temp_str, "%02d ", temperatura);
        ...
        RTC_sec = I2C_Master_Read(1);
        RTC_min = I2C_Master_Read(1);
        RTC_hor = I2C_Master_Read(0);
        I2C_Master_Stop();
        _delay((unsigned long)((100)*(1000000/4000.0)));
        RTC_sec = binTodec(RTC_sec);
        RTC_min = binTodec(RTC_min);
        RTC_hor = binTodec(RTC_hor);
        sprintf(RTC_sec_str, "%02d", RTC_sec);
        sprintf(RTC_min_str, "%02d", RTC_min);
        sprintf(RTC_hor_str, "%02d", RTC_hor);
        ...
        if (temperatura <= 25 && temperatura == 255){
            temp_flag = 52;
        }
        else if (temperatura > 26 && temperatura < 32){
            temp_flag = 53;
        }
        if (disp == 0){
        ...
        usartDataWrite(HS_flag);
        usartDataWrite(spst_flag);
        usartDataWrite(temp_flag);
...
int percent(uint16_t val, uint16_t water_val, uint16_t air_val){
    return (int)(((air_val-val)*100)/(air_val-water_val));
}
